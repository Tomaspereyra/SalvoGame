...
public class SalvoApplication {
	...
	@Bean
	public CommandLineRunner initData(PlayerRepository playerRepository, GameRepository gameRepository, GamePlayerRepository gamePlayerRepo){
		return (args)->{
					...
					gamePlayerRepo.save(new GamePlayer(gameRepository.findById(1).get(),playerRepository.findById(1).get(), new Date()));
					gamePlayerRepo.save(new GamePlayer(gameRepository.findById(1).get(),playerRepository.findById(2).get(), new Date()));
